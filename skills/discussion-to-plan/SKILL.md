...
5. **Red-team before you commit — up to two rounds, revise-and-re-attack.** Before a substantive decision (or a small batch of them) graduates into the plan, stress-test it.
   ...
   - **Close with a TLDR readout.** Once the loop settles, recap what the red-team surfaced for a fast scan, in the TL;DR block from `${CLAUDE_PLUGIN_ROOT}/docs/tldr-convention.md` (rule · blockquote · rule, plain language). This readout is the convention's **one documented exception** — it carries grounding lines, and it is **not capped at 5 bullets**: list every surviving objection, because a summary that drops one defeats the gate. If that runs long, that is the signal, not a formatting problem. Its bullets are still **≤10 words each**, and under each bullet a one-line grounding — a `file:line`, *or* a named condition when the finding is a gap or a cross-artifact relation with no single line — naming the mechanism in a clause. A bare locator is not the grounding; the *why* is, so never let the line number stand in for it. Frame it as a readout of the surviving objections and how each was resolved (folded in / residual accepted), **not** a go/no-go verdict — the decision stays the user's.
   - **Optional contrarian pass (opt-in, OFF by default).** If you notice yourself *capitulating* to critique — folding to objections that only look fatal — you may run `memento:decision-defender` after an adversarial round. It steelmans the case *for* the decision, surfaced **alongside** the surviving objections (it never softens or drops any; you still break ties). This is an **A/B experiment**, not part of the standard gate: use it to see whether a dedicated contrarian changes which objections you revise for. Leave it off unless you're deliberately probing capitulation bias — it spends extra tokens and the default loop already keeps the push-back honest by re-attacking your revision.
6. Only after alignment and review, draft the plan. **Lead the hand-off with a TL;DR block** — ≤5 bullets, ≤10 words each, no jargon, paths, or symbol names, ending on the call you need from the user (`${CLAUDE_PLUGIN_ROOT}/docs/tldr-convention.md`); the plan itself is the detail below it. When this feeds the kit's workflow, write it into the workspace's `plan.md` (from `${CLAUDE_PLUGIN_ROOT}/templates/workflow/plan.md`): a north-star goal, non-scope, and ordered milestones — each milestone carrying an inline user-testable gate (web → page/button · CLI → command + output · lib → invokable runner). Otherwise a short freeform plan (problem, goal, scope, approach, risks, next steps) is fine. Keep it short unless the user asks for depth.
...
- A TL;DR block leading the plan hand-off — ≤5 jargon-free bullets, ≤10 words each, ending on the call needed from the user
...
- A closing TLDR readout of the red-team, in the convention's block: ≤10-word bullets recapping the surviving objections and their resolution, each grounded (a `file:line` or a named condition) — a readout, not a verdict
